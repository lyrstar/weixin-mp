...
const _message = require('./_message');
...
class WeChatMP{
    ...
    constructor(options){
        ...
        if(options.initToken !== true) this.config.initToken = false;
        if(options.refresh !== true) this.config.refresh = false;
    ...
    getJsapiTicket(){
        ...
        let data = {access_token : this.config.accessToken, type : 'jsapi'};
        return httpsGet(USERHONE_API_WEIXIN, 443, path, data);
    }
    ...
    //用户标签管理
    //设置用户备注名
    //获取用户基本信息(UnionID机制) access_token不同于全局的access_token
    getUserInfo(openId, access_token){
        let path = '/cgi-bin/user/info';
        let data = {access_token: access_token, openid: openId, lang : 'zh_CN'};
        return httpsGet(USERHONE_API_WEIXIN, 443, path, data);
    }
    //获取用户列表
