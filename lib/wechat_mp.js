...
class WeChatMP{
    ...
    constructor(options){
        ...
        if(this.config.initToken) this.createAccessToken();
        if(!this.config.initToken && this.config.refresh) this.openRefreshAccessToken();
    ...
    createAccessToken(){
        if(Date.now() - this.tokenRefreshTime < 10000){
            console.error('weixin-mp : createAccessToken : 获取accessToken 时间间隔小于10s');
        }
        this.getAccessToken().
            then(d => {
                console.log('weixin-mp : createAccessToken : ', d);
            ...
            catch(e => console.error('weixin-mp : createAccessToken : error : ', e));
    }
    //定时刷新accessToken
    ...
    //验证accessToken是否有效
    verifyAccessToken(){
        console.log('weixin-mp : setAccessToken : ', this.config.accessToken);
        this.getWeChatServerIp()
            .then(data => {
                if(!data.ip_list) console.error('weixin-mp : verifyAccessToken : error : ', data);
            })
            .catch(err => {
                console.error('weixin-mp : verifyAccessToken : error : ', err);
                if(this.config.fixToken && err.errcode === 40001 ){
                    this.createAccessToken();
                    console.error('weixin-mp : verifyAccessToken : : access_token无效, 已重新获取access_token');
                }
            })
    }
